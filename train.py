...
from custom.recorder import record, init_recorder
...
def training(args_param, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, logger=None, ply_path=None):
    first_iter = 0
    ...
    gaussians.set_es_params(args_param)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), logger, args_param.model_path)
    ...
    gaussians.save_min_max(os.path.join(args_param.model_path, "bitstreams"))
    return
...
def training_report(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, logger=None, pre_path_name=''):
    if tb_writer:
...
def render_sets(args_param, dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train=True, skip_test=False, tb_writer=None, dataset_name=None, logger=None):
    with torch.no_grad():
        gaussians = GaussianModel(
            dataset.feat_dim,
            dataset.n_offsets,
        ...
        gaussians.eval()
        gaussians.load_min_max(os.path.join(args_param.model_path, "bitstreams"))
...
def main():
    ...
    parser.add_argument("--step_begin_entropy_skipping", type=int, default=30001)
    parser.add_argument("--enable_es_in_final_estimation", type=int, default=0)
    parser.add_argument("--es_ratio", type=float, default=0.5)
    ...
    # training
    training(args, lp.extract(args), op.extract(args), pp.extract(args), dataset,  args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, logger)
    logger.info("\nTraining complete.")

    # rendering
    logger.info(f'\nStarting Rendering~')
    visible_count = render_sets(args, lp.extract(args), -1, pp.extract(args),  logger=logger)
    logger.info("\nRendering complete.")
